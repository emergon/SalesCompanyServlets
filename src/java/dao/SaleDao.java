...
public class SaleDao extends SuperDao implements InterfaceDao<Sale> {
    ...
    private static final String INSERTSALE = "INSERT INTO sales (smcode, ccode, pcode, quant, cost, sdate) VALUES (?,?,?,?,?,?)";

    
    public boolean create(int salesman, int customer, int product, int quant, double cost, LocalDate date) {
        PreparedStatement pst = null;
        boolean created = false;
        try {
            pst = getConnection().prepareStatement(INSERTSALE);
            pst.setInt(1, salesman);
